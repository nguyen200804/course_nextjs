...
export default async function TopicDetailPage({ params }: PageProps) {
  ...
  try {
    ...
    // Fetch parent lesson and topic from WordPress REST API
    try {
      ...
      activeLesson = lessonRes.ok ? await lessonRes.json() : { id: lessonId, title: { rendered: `Lesson #${lessonId}` } };
      activeTopic = topicRes.ok ? await topicRes.json() : { id: topicId, title: { rendered: `Topic #${topicId}` }, content: { rendered: `<p>Topic content is being updated...</p>` } };
    } catch {
      activeLesson = { id: lessonId, title: { rendered: `Lesson #${lessonId}` } };
      activeTopic = { id: topicId, title: { rendered: `Topic #${topicId}` }, content: { rendered: `<p>Topic content is being updated...</p>` } };
    }
    ...
    error = err.message || "Could not load details for this topic.";
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center shadow-2xl">
          ...
          <h2 className="text-xl font-bold text-red-300 mb-3">Error Loading Topic</h2>
          ...
          <div className="lg:col-span-3 space-y-8">
            <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 md:p-10 backdrop-blur-xl shadow-xl">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500 block mb-3">
                Topic / {activeLesson?.title?.rendered}
              </span>
              <h1 
              ...
              {canAccess ? (
                ...
                <div className="border border-slate-800/60 rounded-2xl bg-slate-950/40 p-8 text-center my-6 shadow-inner">
                  ...
                  <h3 className="text-lg font-bold text-red-400 mb-2">This topic requires enrollment</h3>
                  <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">
                    Please enroll or purchase the course to unlock all lesson content.
                  </p>
                  <Link 
                    href={`/courses/${slug}`} 
