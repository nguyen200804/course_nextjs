...
export async function GET(req: Request) {
  try {
    ...
    // Fallback: call LearnPress REST API
    try {
      let res = await fetch(`${wpUrl}/wp-json/learnpress/v1/quiz/${encodeURIComponent(quizId)}`, { headers });
      if (!res.ok) {
        res = await fetch(`${wpUrl}/wp-json/wp/v2/lp_quiz/${encodeURIComponent(quizId)}?_embed=true`, { headers });
      }
      if (!res.ok) {
        res = await fetch(`${wpUrl}/wp-json/wp/v2/lp_quiz?slug=${encodeURIComponent(quizId)}&_embed=true`, { headers });
      }
          ...
          const questions = qList.map((item: any, idx: number) => {
            ...
            if (Array.isArray(item.options)) {
              item.options.forEach((opt: any, optIdx: number) => {
                const optTitle = typeof opt === "object" ? (opt.title || opt.value || opt.label || opt.text || "") : String(opt);
                rawOptions.push(optTitle);
                if (typeof opt === "object") {
                  const isTrue = opt.is_true === "yes" || opt.is_true === "1" || opt.is_true === 1 || opt.is_true === true || opt.is_correct === true || opt.is_correct === "yes";
                  if (isTrue) {
            ...
            let correctVal: any = 0;
            if (isMulti) {
              correctVal = correctIndices;
            } else {
              correctVal = correctIndices.length > 0 ? correctIndices[0] : (item.correct !== undefined ? item.correct : 0);
            }
